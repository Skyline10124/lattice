...
impl<'a> MessageWidget<'a> {
    ...
    pub fn render(&self, area: ratatui::layout::Rect, buf: &mut ratatui::buffer::Buffer) {
        let prefix = match self.msg.role {
            lattice_core::types::Role::User => "\u{F2BD} ",
            lattice_core::types::Role::Assistant => "\u{F120} ",
            lattice_core::types::Role::System => "\u{F013} ",
            lattice_core::types::Role::Tool => "\u{F0AD} ",
        };

        let style = match self.msg.role {
            lattice_core::types::Role::User => self.theme.user_style(),
            lattice_core::types::Role::Assistant => self.theme.assistant_style(),
            lattice_core::types::Role::System => Style::default().fg(self.theme.subtext),
            lattice_core::types::Role::Tool => self.theme.tool_style(),
        };
