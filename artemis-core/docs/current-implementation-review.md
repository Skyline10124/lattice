...
6. `artemis-cli` crate 编译失败（16 个 E0583 模块缺失）。（W-1）
7. `artemis-tui` 在 Cargo.toml 但目录不存在。（W-2）
...
|------|------|------------|---------|
| P0-2 | `artemis-core/pyproject.toml` 遗留 | `9e7250a` | 文件已删 |
...
#### W-1：`artemis-cli` 编译失败

`Cargo.toml` 包含 `artemis-cli` member，但 `src/commands/` 下有 5 个模块（`config_cmd`, `models`, `sessions`, `stats`, `config`）缺失源文件，导致 16 个 E0583 编译错误。`main.rs` 和 `print.rs` / `resolve.rs` / `doctor.rs` / `session.rs` / `credentials.rs` / `config.rs` 存在。

这是新建的 CLI crate skeleton，部分命令模块尚未实现。

建议：要么补齐 stub 模块，要么从 Cargo.toml 暂时移除。

#### W-2：`artemis-tui` 在 Cargo.toml 但目录不存在

workspace members 列出 `artemis-tui`，但没有对应目录。导致 `cargo test --workspace` 无法运行。
...
| Crate | 职责 | 状态 |
|-------|------|------|
| `artemis-core` | model routing、HTTP/SSE、transport、streaming、errors、tokens | ✅ 核心路径 runtime 基本闭环 |
| `artemis-agent` | Agent state、tool boundary、retry | ⚠️ setter 存在，memory/token_pool 未接入 |
| `artemis-memory` | `Memory` trait + `InMemoryMemory` | ✅ 3 tests |
| `artemis-token-pool` | `TokenPool` trait + `UnlimitedPool` | ✅ 3 tests |
| `artemis-python` | PyO3 binding | ⚠️ 仅 resolver |
| `artemis-cli` | CLI interface | ❌ 编译失败 |
| `artemis-tui` | TUI interface | ❌ 目录不存在 |
...
|-------|--------|---------|------|
| artemis-core (lib) | 170 | 1 | |
| artemis-core (e2e) | 170 | 0 | |
| artemis-core (transport_char) | 29 | 0 | |
| artemis-core (transport_integration) | 42 | 0 | |
| artemis-core (examples) | 0 | 4 | ignored |
| artemis-agent | 0 | 0 | 无 #[test] |
| artemis-memory | 3 | 0 | |
| artemis-token-pool | 3 | 0 | |
| artemis-python | 0 | 0 | |
| **总计** | **414** | **5** | |

文档测试计数已对齐（testing.md: core 411, agent 0）。

注意：`cargo test --workspace` 因 artemis-tui 缺失而无法运行。需先移除或补齐。
...
1. W-2 — 移除 `artemis-tui` 从 Cargo.toml（或创建空 crate）
2. W-1 — 补齐 artemis-cli stub 模块（或暂时从 workspace exclude）
