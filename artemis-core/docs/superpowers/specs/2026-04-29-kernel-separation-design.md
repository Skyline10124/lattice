# artemis Kernel Separation Design
...
Split the monolithic `artemis-core` crate into five focused crates following
...
```
artemis-python        PyO3 bindings, exposes all four lower crates to Python
    ↓
artemis-agent         AgentLoop, conversation state, tool boundary
    ↓
artemis-memory        Cross-session persistent memory (trait)
artemis-token-pool    Multi-agent shared token budget (trait)
    ↓
artemis-core          Model resolution + streaming inference
```
...
tokio runtime. Callers (artemis-agent, artemis-python) bring their runtime
...
- `artemis-core` (for `resolve`, `chat`, types)
- `artemis-memory` (optional trait)
- `artemis-token-pool` (optional trait)
...
None (standalone trait, no artemis-core types needed).
...
```python
import artemis_core  # crate name stays artemis_core for pip compatibility

# Model resolution
resolved = artemis_core.resolve("sonnet")

# Single-turn chat
for event in artemis_core.chat(resolved, messages):
    print(event)

# Multi-turn agent
agent = artemis_core.Agent(resolved)
...
| From current `artemis-core/src/` | To |
|------|----|
| `agent_loop.rs` | `artemis-agent/src/loop_.rs` |
| `tool_boundary.rs` | `artemis-agent/src/tool_boundary.rs` |
| `streaming_bridge.rs` | `artemis-python/src/streaming_bridge.rs` |
| `engine.rs` | Split three ways: 1) `ArtemisEngine` PyClass + `Event`/`ToolCallInfo`/`PyResolvedModel` PyO3 types → `artemis-python/src/engine.rs`; 2) `run_once()`/`run_conversation()`/`submit_tool_result[s]()` → `artemis-agent/src/run.rs` (merged into `Agent::send`/`submit_tools`); 3) `resolve_model()`/`list_models()`/`register_model()` already handled by `router.rs` in core — nothing to move |
| `mock.rs` | `artemis-agent/` (only used by agent tests; not needed in core) |
| Everything else | Stays in `artemis-core` |
...
- All crates use `artemis_core::ArtemisError` as the error type. No new error enums.
