...
**Scope**: Full Rust workspace (artemis-core, artemis-agent, artemis-memory, artemis-token-pool, artemis-plugin, artemis-harness, artemis-cli, artemis-tui, artemis-python)
...
error: this operation has no effect
  --> artemis-agent/src/sandbox.rs:47:29
   |
...
**File**: `artemis-agent/src/lib.rs:199-200`
...
**File**: `artemis-agent/src/tools.rs:478-493`
...
**File**: `artemis-memory/src/lib.rs:104-155`
...
**File**: `artemis-core/src/logging.rs:56`
...
**File**: `artemis-core/src/logging.rs:16-37`
...
**File**: `artemis-core/src/transport/gemini.rs:254,355`
...
**File**: `artemis-core/src/lib.rs:148-151`
...
**File**: `artemis-agent/src/lib.rs:152-195`
...
**File**: `artemis-agent/src/lib.rs:354-358`
...
**File**: `artemis-core/src/transport/mod.rs:127-130`
...
**File**: `artemis-memory/src/lib.rs:51`
...
**File**: `artemis-core/src/router.rs:438`
...
**File**: `artemis-core/src/provider.rs:8-13`
...
**File**: `artemis-core/src/errors.rs:164`
...
```
artemis-python → artemis-agent → artemis-memory → artemis-token-pool → artemis-core
                                     ↘ artemis-plugin ↗
artemis-harness → artemis-agent + artemis-memory
artemis-cli → artemis-agent + artemis-core
artemis-tui → artemis-agent
```

No circular deps. Each crate has a clear responsibility.

### Concern: `artemis-core::chat()` is doing too much
...
### Concern: `artemis-agent::tools` mixes tool definitions with tool execution
...
|-------|-----------|-----------|-----------------|
| artemis-core | Good (errors, streaming, transport, router) | 3 failing | Solid, but no test for `chat()` itself |
| artemis-agent | State tests only | None | Gaps: no test for `run()`, `chat_with_retry()`, tool execution |
| artemis-memory | InMemory + SQLite basic | None | No concurrency test for global store |
| artemis-harness | None | None | No test coverage at all |
| artemis-cli | None | None | No test coverage |
...
| Temp file cleanup | `execute_code` leaves files in `/tmp` | **P3** — no cleanup of `artemis_code_*` dirs |
...
|----|-------|-----------|-------------|
| C1 | Blocker | artemis-agent | Clippy: `identity_op` in sandbox.rs |
...
| N1 | P0 | artemis-agent | UTF-8 panic in `Agent::run()` memory save |
| N2 | P0 | artemis-agent | Command injection via prefix-matched allowlist |
| N3 | P0 | artemis-memory | Global state in `InMemoryMemory` causes test pollution |
| N4 | P1 | artemis-core | `init_debug_logging` panics on file open |
| N5 | P1 | artemis-core | `init_logging` silently ignores second call |
| N6 | P1 | artemis-core | Gemini random tool call IDs break idempotency |
| N7 | P1 | artemis-core | SSE connection errors not classified |
| N8 | P2 | artemis-agent | Mid-stream errors not retried |
| N9 | P2 | artemis-agent | `chat_with_retry` clones messages per attempt |
| N10 | P2 | artemis-core | Deprecated `denormalize_stream_chunk` still implemented |
| N11 | P2 | artemis-memory | `async_trait` overhead for sync implementations |
| N12 | P3 | artemis-core | `resolve_permissive` doesn't lowercase model_part |
| N13 | P3 | artemis-core | HTTP read timeout unclear for SSE |
| N14 | P3 | artemis-core | `ErrorClassifier` only checks one 400 pattern |
