...
use artemis_plugin::{CodeReviewPlugin, Plugin, PluginAgent, PluginConfig, PluginRunner, StrictBehavior};
use std::fs;
...
fn main() {
    ...
    for path in &["artemis-cli/src/main.rs", "artemis-tui/src/app.rs", "artemis-tui/src/main.rs"] {
        if let Ok(content) = fs::read_to_string(&format!("/home/astrin/artemis/{}", path)) {
            code.push_str(&format!("\n=== {} ===\n{}", path, &content[..content.len().min(3000)]));
        }
    }
    ...
    let behavior = StrictBehavior { confidence_threshold: 0.6, max_retries: 2, escalate_to: None };
