...
///
/// Handles HTTP communication with the OpenAI API, using
...
impl OpenAIProvider {
    /// Create a new OpenAIProvider with a default transport.
    pub fn new() -> Self {
        OpenAIProvider {
    ...
    /// Create an OpenAIProvider with a custom transport (e.g. for proxied endpoints).
...
impl Provider for OpenAIProvider {
    ...
    async fn chat_stream(&self, _request: ChatRequest) -> Result<EventStream, ProviderError> {
        Err(ProviderError::Stream(
            "SSE streaming not yet implemented for OpenAI provider".into(),
        ))
    }
...
mod tests {
    ...
    fn make_simple_request() -> ChatRequest {
        ...
        let resolved = make_resolved("gpt-4o");
        ChatRequest {
            messages,
            tools: vec![],
    ...
    fn test_with_custom_transport() {
        ...
        let p = OpenAIProvider::with_transport(transport);
        assert_eq!(p.name(), "openai");
    }
    ...
    fn test_request_uses_resolved_api_model_id() {
        let request = make_simple_request();
        assert_eq!(request.resolved.api_model_id, "gpt-4o");
    ...
    fn test_default_base_url_when_empty() {
        let p = OpenAIProvider::new();
        assert_eq!(p.transport.base_url(), "https://api.openai.com/v1");
    }
