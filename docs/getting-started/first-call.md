...
async fn main() {
    // 1. 解析模型
    let resolved = lattice_core::resolve("deepseek-v4-pro")?;
    ...
    // 3. 非流式调用
    let response = lattice_core::chat_complete(&resolved, &[msg], &[]).await?;
...
```rust
let tools = vec![lattice_core::ToolDefinition {
...
let response = lattice_core::chat_complete(&resolved, &messages, &tools).await?;
