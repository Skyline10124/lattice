...
| **垂类精通** | lattice-core 只做模型路由 + 推理，不碰 agent logic |

**与其他项目的区别**：

| 项目 | 是什么 | 和 LATTICE 的关系 |
|------|--------|------------------|
| **OpenRouter** | 模型路由 SaaS | 竞品，但 LATTICE 是库不是 SaaS |
| **LiteLLM** | 模型网关 | 竞品，但 LATTICE 是 Rust 不是 Python |
| **LangGraph / CrewAI** | 多 agent 编排框架 | 不同层，重框架 vs 轻库 |
| **n8n / Dify** | 可视化工作流 | 不同赛道，业务自动化 vs 开发者工具 |
| **Google A2A** | Agent 间通信协议 | 参考，LATTICE 的 handoff 层可以参考 |
...
│    pip install lattice-code-review-plugin   │
├───────────────────────────────────────────┤
│    lattice-core（Rust，最小内核）            │
...
| 层 | 职责 | 信任 |
|----|------|------|
| **lattice-core** | 模型路由、HTTP、SSE、retry、token 估算 | 持凭证 |
...
```
LATTICE 插件：agent 的完整定义（Input/Output + prompt + tools + handoff + behavior）
MCP：        工具的协议（如何暴露、调用、返回），插件内部通过 MCP 引用工具
CLI：        无 AI 的纯代码工具
```

三层各管各的：LATTICE 管 agent 组装和行为，MCP 管工具协议，CLI 不涉及 AI。
...
- [x] C2: Agent 独立 crate（lattice-agent）
...
- [x] lattice-tui: Ratatui TUI + Agent streaming
- [x] 凭证缺失时报错（P2-1）
- [x] 9 crates, ~440+ 测试

### 第四阶段：类型化插件系统（进行中）

- [x] Plugin trait: Input/Output 类型接口定义
- [x] Behavior trait: Strict / Yolo
- [x] PluginRunner, PluginHooks, PluginConfig
- [x] CodeReviewPlugin（内置）
- [ ] `to_prompt()` / `from_output()` trait 正式化
- [ ] 输出解析 + 校验 + 重试框架
- [ ] Python 胶水层：插件加载 + 组合
- [ ] lattice-agent-protocol：handoff 路由
...
| [OpenRouter](https://openrouter.ai) | 模型路由 SaaS | lattice-core 对标 |
